...
学习难度，开发效率，维护性这些方面也不逊色于Ruby on rails, Django。
...
2. Fluent形式的DAO  
   根据数据结构定义XML文件，可以自动生成关于表创建的SQL，数据存储的SQL和JAVA源代码。
   利用这些自动生成的代码，可以很方便的书写数据存储的业务逻辑代码。
   不需要编写SQL（※复杂的SQL除外）

3. 业务逻辑的开发简单  
   开发人员利用Panda Framework的各种功能，可以集中精力开发业务逻辑。
   「检索、列表、显示、追加、变更、删除」这样的基本功能都已经被模板化，利用这些模板可以很轻松的制作业务逻辑。
...
5. 各种有用的现成模块  
   * HTML画面⇒PDF报表转换功能  
     ...
     使用HTML⇒PDF转换功能， 可以直接把HTML画面转换为PDF报表，大幅削减开发费用。  
   * 图片管理模块
   * 投稿管理模块
...
Panda Framework由以下模块构成。
 ...
 | [panda-gems](gems/index_zh.md) | Web-APP扩展模块。包含了实现CRUD的基础模板类，用户认证/权限管理的基础类，和一些其他的有用的类。|
 ...
 | [panda-cgen](cgen/index_zh.md) | CRUD源代码生成器。可以自动生成Entity类, Query类, Dao类, Action类, View的Freemarker(HTML)模板文件。 |
...
我做了一个[DEMO网站](https://panda-demo.azurewebsites.net)，通过这个DEMO大家可以比较直观的了解Panda Framework。
