...
import panda.codec.BinaryDecoder;
 ...
 * correspond to printable characters in the ASCII character set. It encodes the data in such a way
 * that the resulting octets are unlikely to be modified by mail transport. If the data being
 ...
 * Note:
 * <p>
 * Depending on the selected {@code strict} parameter, this class will implement a different set of
 * rules of the quoted-printable spec:
 * <ul>
 * <li>{@code strict=false}: only rules #1 and #2 are implemented
 * <li>{@code strict=true}: all rules #1 through #5 are implemented
 * </ul>
 * Originally, this class only supported the non-strict mode, but the codec in this partial form
...
public class QuotedPrintableCodec implements BinaryEncoder, BinaryDecoder, StringEncoder, StringDecoder {
	 ...
	 */
	public QuotedPrintableCodec(final String charsetName) throws IllegalCharsetNameException, IllegalArgumentException,
	...
	/**
	 * Return the byte at position <code>index</code> of the byte array and make sure it is
	 * unsigned.
	 * 
	 ...
	 * <p>
	 * This function implements a subset of quoted-printable encoding specification (rule #1 and
	 ...
	 * the full ruleset or only a subset of quoted-printable encoding specification (rule #1 and
	 * rule #2) as defined in RFC 1521 and is suitable for encoding binary data and unformatted
	 * text.
	 * 
	 * @param printable bitset of characters deemed quoted-printable
	...
	public String decode(final String str, final Charset charset) throws DecoderException {
		...
		return new String(this.decode(Strings.getBytesUsAscii(str)), charset);
	}
	...
	public String encode(final String str, final Charset charset) {
		...
		return Strings.newStringUsAscii(this.encode(str.getBytes(charset)));
	}
