...
/***
 * The TimeUDPClient class is a UDP implementation of a client for the Time protocol described in
 * RFC 868. To use the class, merely open a local datagram socket with
 * {@link panda.net.DatagramSocketClient#open open } and call {@link #getTime getTime } or
 * {@link #getTime getDate } to retrieve the time. Then call
 * {@link panda.net.DatagramSocketClient#close close } to close the connection properly. Unlike
 * {@link panda.net.time.TimeTCPClient}, successive calls to {@link #getTime getTime } or
 * {@link #getDate getDate } are permitted without re-establishing a connection. That is because UDP
 * is a connectionless protocol and the Time protocol is stateless.
 * 
 * @see TimeTCPClient
 ***/
...
public final class TimeUDPClient extends DatagramSocketClient {
	...
	/***
	 * Retrieves the time from the specified server and port and returns it. The time is the number
	 * of seconds since 00:00 (midnight) 1 January 1900 GMT, as specified by RFC 868. This method
	 * reads the raw 32-bit big-endian unsigned integer from the server, converts it to a Java long,
