...
public class IMAP extends SocketClient {
	...
	/**
	 * Implement this interface and register it via {@link #setChunkListener(IMAPChunkListener)} in
	 * order to get access to multi-line partial command responses. Useful when processing large
	 * FETCH responses.
	 */
	public interface IMAPChunkListener {
		 ...
		 * 
		 * @param imap the instance, get the response by calling {@link #getReplyString()} or
		 *            {@link #getReplyStrings()}
	 ...
	 * </p>
	 * <p>
	 * This is intended for use with a suitable ProtocolCommandListener. If the IMAP response
	 * contains multiple-line data, the protocol listener will be called for each multi-line chunk.
	 * The accumulated reply data will be cleared after calling the listener. If the response is
	 * very long, this can significantly reduce memory requirements. The listener will also start
	 * receiving response data earlier, as it does not have to wait for the entire response to be
	 * read.
	 * </p>
	 * <p>
	 * The ProtocolCommandListener must be prepared to accept partial responses. This should not be
	 * a problem for listeners that just log the input.
	 * </p>
	 * 
	...
	/**
	 * Overrides {@link SocketClient#fireReplyReceived(int, String)} so as to avoid creating the
	 * reply string if there are no listeners to invoke.
	 * 
	 ...
	 * <code> DISCONNECTED_STATE </code>. The reply text information from the last issued command is
	 * voided to allow garbage collection of the memory used to store that information.
	 * 
	...
	/**
	 * Returns an array of lines received as a reply to the last command sent to the server. The
	 * lines have end of lines truncated.
	 * 
	...
	/**
	 * Returns the reply to the last command sent to the server. The value is a single string
	 * containing all the reply lines including newlines.
	 * 
	 * @return The last server response.
	 */
	...
	/**
	 * Sets the current chunk listener. If a listener is registered and the implementation returns
	 * true, then any registered {@link panda.net.PrintCommandListener PrintCommandListener}
	 * instances will be invoked with the partial response and a status of {@link IMAPReply#PARTIAL}
	 * to indicate that the final reply code is not yet known.
	 * 
