...
public class AuthenticatingSMTPClient extends SMTPSClient {
	...
	/**
	 * Overloaded constructor that takes the protocol specification, the implicit argument and
	 * encoding
	 * 
	 * @param proto the protocol.
	 * @param implicit The security mode, {@code true} for implicit, {@code false} for explicit
	...
	/**
	 * Overloaded constructor that takes the implicit argument, and using {@link #DEFAULT_PROTOCOL}
	 * i.e. TLS
	 * 
	 * @param implicit The security mode, {@code true} for implicit, {@code false} for explicit
	...
	/***
	 * A convenience method to send the ESMTP EHLO command to the server, receive the reply, and
	...
	/***
	 * Login to the ESMTP server by sending the EHLO command with the client hostname as an
	 * argument. Before performing any mail commands, you must first login.
	 * <p>
	 * 
	 * @return True if successfully completed, false if not.
	 * @exception SMTPConnectionClosedException If the SMTP server prematurely closes the connection
	 ...
	 * 
	 * @return The integer values of the enhanced reply code of the last SMTP reply. First digit is
	...
	/***
	 * Authenticate to the SMTP server by sending the AUTH command with the selected mechanism,
	 * using the given username and the given password.
	 * 
	 * @param method the method to use, one of the {@link AuthenticatingSMTPClient.AUTH_METHOD} enum
	 *            values
	 * @param username the user name. If the method is XOAUTH, then this is used as the plain text
	 *            oauth protocol parameter string which is Base64-encoded for transmission.
	 * @param password the password for the username. Ignored for XOAUTH.
	 * @return True if successfully completed, false if not.
	 * @exception SMTPConnectionClosedException If the SMTP server prematurely closes the connection
	 *                as a result of the client being idle or some other reason causing the server
	 *                to send SMTP reply code 421. This exception may be caught either as an
	 ...
	 *                password.
	 ***/
	public boolean auth(AuthenticatingSMTPClient.AUTH_METHOD method, String username, String password)
		...
		if (method.equals(AUTH_METHOD.PLAIN)) {
			...
			// Java 1.6 can use getCharset()
				Base64.encodeBase64String(("\000" + username + "\000" + password).getBytes(getCharsetName()))));
		}
			...
			return SMTPReply.isPositiveCompletion(sendCommand(Base64.encodeBase64String(toEncode)));
		}
			...
			if (!SMTPReply.isPositiveIntermediate(sendCommand(Base64.encodeBase64String(username
				.getBytes(getCharsetName()))))) { // Java 1.6 can use getCharset()
				return false;
			}
			return SMTPReply.isPositiveCompletion(sendCommand(Base64.encodeBase64String(password
				.getBytes(getCharsetName())))); // Java 1.6 can use getCharset()
		}
		else if (method.equals(AUTH_METHOD.XOAUTH)) {
			return SMTPReply.isPositiveIntermediate(sendCommand(Base64.encodeBase64String(username
				.getBytes(getCharsetName())) // Java 1.6 can use getCharset()
				));
		}
		else {
