...
public class Main {
	...
	/**
	 * Helper application for example classes. Lists available classes, and provides shorthand
	 * invocation. For example:<br>
	 * <code>java -jar commons-net-examples-m.n.jar FTPClientExample -l host user password</code>
	 * 
	 * @param args the first argument is used to name the class; remaining arguments are passed to
	 *            the target class.
	...
	public static void main(String[] args) throws Throwable {
		...
		if (args.length == 0) {
			if (Thread.currentThread().getStackTrace().length > 2) { // called by Maven
				...
				if (fromJar()) {
					System.out
						.println("Usage: java -jar commons-net-examples-m.n.jar <alias or exampleClass> <exampleClass parameters>");
				}
				else {
					System.out
						.println("Usage: java -cp target/classes examples/Main <alias or exampleClass> <exampleClass parameters>");
				}
			}
