...
<div class="index-sorter">
	...
	<#list action.orders?keys as _sc>
		...
		<#if ((params.s.c)!'') == _sc>
		<button class="btn btn-primary" onclick="site.page_sort_reverse('${_sc}', '${(params.s.d)!}'); return false;"
			><@p.text name="lbl-sort-${_sc}"/> <i class="fa fa-sort-${(params.s.d)!}"></i></button>
		<#else>
		<button class="btn btn-default" onclick="site.page_sort('${_sc}', '${_sd}'); return false;"
			><@p.text name="lbl-sort-${_sc}"/> <i class="fa fa-sort-${(params.s.d)!}"></i></button>
		</#if>
	</#list>
	</div>
