 ...
 * <code>readNextEntry()</code> - which handles the issue of what delimits one entry from another,
 * usually but not always a line feed and <code>preParse()</code> - which handles removal of
 ...
 * This may be done either in block mode, by calling the <code>getNext()</code> and
 * <code>getPrevious()</code> methods to provide "paged" output of less than the whole list at one
 * time, or by calling the <code>getFiles()</code> method to return the entire list.
 * <p>
 * Examples:
 * <p>
 * Paged access:
...
public class FTPListParseEngine {
	...
	/**
	 * Internal method for reading the input into the <code>entries</code> list. After this method
	 * has completed, <code>entries</code> will contain a collection of entries (as defined by
	 * <code>FTPFileEntryParser.readNextEntry()</code>), but this may contain various non-entry
	...
	/**
	 * Returns an array of at most <code>quantityRequested</code> FTPFile objects starting at this
	 * object's internal iterator's current position. If fewer than <code>quantityRequested</code>
	 ...
	 * @return an array of at most <code>quantityRequested</code> FTPFile objects starting at the
	 ...
	 * fewer than <code>quantityRequested</code> such elements are available, the returned array
