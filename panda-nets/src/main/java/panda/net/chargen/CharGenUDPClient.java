 ...
 * {@link panda.net.DatagramSocketClient#open open } and call {@link #send send } to send the
 * datagram that will initiate the data reply. For chargen or quote of the day, just call
 * {@link #receive receive }, and you're done. For netstat and systat, call receive in a while loop,
 * and catch a SocketException and InterruptedIOException to detect a timeout (don't forget to set
 * the timeout duration beforehand). Don't forget to call
...
public final class CharGenUDPClient extends DatagramSocketClient {
	...
	/***
	 * The default CharGenUDPClient constructor. It initializes some internal data structures for
	 * sending and receiving the necessary datagrams for the chargen and related protocols.
	 ***/
	...
	/***
	 * Sends the data initiation datagram. This data in the packet is ignored by the server, and
	 * merely serves to signal that the server should send its reply.
	 * 
	...
	/***
	 * Receive the reply data from the server. This will always be 512 bytes or less. Chargen and
	 * quote of the day only return one packet. Netstat and systat require multiple calls to
	 * receive() with timeout detection.
	 * 
