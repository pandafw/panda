package panda.net.examples.ntp;
...
/**
 * The SimpleNTPServer class is a UDP implementation of a server for the Network Time Protocol (NTP)
 * version 3 as described in RFC 1305. It is a minimal NTP server that doesn't actually adjust the
 * time but only responds to NTP datagram requests with response sent back to originating host with
 * info filled out using the current clock time. To be used for debugging or testing. To prevent
...
public class SimpleNTPServer implements Runnable {
	 ...
	 * 
	 * @param port the local port the server socket is bound to, or <code>zero</code> for a system
	 *            selected free port.
	...
	/**
	 * Handle incoming packet. If NTP packet is client-mode then respond to that host with a NTP
	 * response packet otherwise ignore.
	 * 
