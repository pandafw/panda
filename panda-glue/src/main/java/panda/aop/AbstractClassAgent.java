...
public abstract class AbstractClassAgent implements ClassAgent {
	...
	protected abstract <T> Class<T> generate(ClassDefiner cd, Pair2[] pair2s, String newName, Class<T> klass,
	...
	protected <T> Constructor<T>[] getEffectiveConstructors(Class<T> klass) {
		...
		if (cList.size() == 0) {
			throw new RuntimeException("No non-private constructor founded,unable to create sub-class!");
		}
	...
	protected <T> boolean checkClass(Class<T> klass) {
		...
		if (klass_name.endsWith(CLASSNAME_SUFFIX))
			return false;
		if (klass.isInterface() || klass.isArray() || klass.isEnum() || klass.isPrimitive() || klass.isMemberClass()
				|| klass.isAnnotation() || klass.isAnonymousClass())
			throw new RuntimeException(klass_name + " is NOT a Top-Class!Creation FAIL!");
		if (Modifier.isFinal(klass.getModifiers()) || Modifier.isAbstract(klass.getModifiers()))
			throw new RuntimeException(klass_name + " is final or abstract!Creation FAIL!");
		return true;
	}
	...
	protected <T> Class<T> try2Load(String newName, ClassDefiner cd) {
		try {
			...
			try {
				...
				try {
					return (Class<T>)Classes.getClass(newName, false);
				}
