...
<#if result?has_content>
	...
	<@p.listview id="resource_benable"
	...
	<div class="p-tcenter" focusme="true">
		<@p.submit onclick="return resource_benable_submit();" icon="icon-benable-execute" label="#(btn-benable-execute)"/>

		<@p.a href="javascript:window.history.back()" btn="default" icon="icon-back" label="#(btn-back)"/>

		<script type="text/javascript"><!--
			function resource_benable_submit() {
				return plv_submitCheckedKeys('resource_benable');
			}
			
			function onPageLoad() {
				plv_checkAll('resource_benable');
			}
		--></script>
	</div>
<#else>
