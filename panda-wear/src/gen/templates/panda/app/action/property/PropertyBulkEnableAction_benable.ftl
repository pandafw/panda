...
<#if result?has_content>
	...
	<@p.listview id="property_benable"
	...
	<div class="p-tcenter" focusme="true">
		<@p.submit onclick="return property_benable_submit();" icon="icon-benable-execute" label="#(btn-benable-execute)"/>

		<@p.a href="javascript:window.history.back()" btn="default" icon="icon-back" label="#(btn-back)"/>

		<script type="text/javascript"><!--
			function property_benable_submit() {
				return plv_submitCheckedKeys('property_benable');
			}
			
			function onPageLoad() {
				plv_checkAll('property_benable');
			}
		--></script>
	</div>
<#else>
