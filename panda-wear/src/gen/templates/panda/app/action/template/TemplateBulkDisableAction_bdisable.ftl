...
<#if result?has_content>
	...
	<div class="p-tcenter" focusme="true">
		<@p.submit onclick="return template_bdisable_submit();" icon="icon-bdisable-execute" label="#(btn-bdisable-execute)"/>

		<@p.a href="javascript:window.history.back()" btn="default" icon="icon-back" label="#(btn-back)"/>

		<script type="text/javascript"><!--
			function template_bdisable_submit() {
				return plv_submitCheckedKeys('template_bdisable');
			}
			
			function onPageLoad() {
				plv_checkAll('template_bdisable');
			}
		--></script>
	</div>
<#else>
