...
public final class FastPageParser {
	...
	public Page parse(char[] data) throws IOException {
		Page page = internalParse(new CharArrayReader(data));
		return page;
	}
	...
	private Page internalParse(Reader reader) {
		...
		while (_tokenType != TOKEN_EOF) {
			if (tagged) {
				if (_tokenType == TOKEN_TAG || _tokenType == TOKEN_EMPTYTAG) {
					...
					if (_buffer.compareLowerSubstr("/content")) // Note that the '/' survives the |
																// 32 operation
					{
						tagged = false;
				...
				if (_tokenType == TOKEN_TAG || _tokenType == TOKEN_EMPTYTAG) {
					...
					if (state == TAG_STATE_XML || state == TAG_STATE_XMP) {
						...
						case HEAD_HASH:
							...
							break;
						case XML_HASH:
							if (!_buffer.compareLowerSubstr("xml")) { // skip any accidental hash
																		// collisions
								doDefault = true;
								break;
							}
							...
							break;
						case XMP_HASH:
							if (!_buffer.compareLowerSubstr("xmp")) { // skip any accidental hash
																		// collisions
								doDefault = true;
								break;
							}
							...
							break;
						case TITLE_HASH:
							if (!_buffer.compareLowerSubstr("title")) { // skip any accidental hash
																		// collisions
								doDefault = true;
								break;
							}
						...
						case SLASH_TITLE_HASH:
							...
							break;
						case PARAMETER_HASH:
							if (!_buffer.compareLowerSubstr("parameter")) { // skip any accidental
																			// hash collisions
								doDefault = true;
								break;
							}
							...
							break;
						case META_HASH:
							if (!_buffer.compareLowerSubstr("meta")) { // skip any accidental hash
																		// collisions
								doDefault = true;
								break;
							}
						...
						case SLASH_HEAD_HASH:
							...
							break;
						case FRAME_HASH:
							if (!_buffer.compareLowerSubstr("frame")) { // skip any accidental hash
																		// collisions
								doDefault = true;
								break;
							}
		...
		return new Page(_title.toString().trim(), _head.toString().trim(), _body.toString().trim());
	}
	...
	/**
	 * Populates a {@link Tag} object using data from the supplied {@link CharArray}. The supplied
	 * tag parameter is reset and reused - this avoids excess object creation which hwlps
	 * performance.
	 * 
	 * @return the same tag instance that was passed in, except it will be populated with a new
	 *         <tt>name</tt> value (and the corresponding <tt>nameEndIdx</tt> value). However if the
	 *         tag contained nathing but whitespace, this method will return <tt>null</tt>.
	 */
	...
	/**
	 * This is called when we need to extract the properties for the tag from the tag's HTML. We
	 * only call this when necessary since it has quite a lot of overhead.
	 * 
	 * @param tag the tag that is currently being processed. This should be the tag that was
	 *            returned as a result of a call to {@link #parseTag(FastPageParser.Tag, CharArray)}
