 ...
 * <p/>
 * It provides basic functionality to handle dynamically-growing char arrays as quickly as possible.
...
public class CharArray {
	...
	/**
	 * Changes the size of the character array to the value specified. If the new size is less than
	 * the current size, the data in the internal array will be truncated. If the new size is &lt;=
	 * 0, the array will be reset to empty (but, unlike StringBuffer, the internal array will NOT be
	 * shrunk). If the new size is &gt the current size, the array will be padded out with null
	 * characters (<tt>'&#92;u0000'</tt>).
	 * 
	...
	/**
	 * Allows an arbitrary substring of this character array to be specified. This method should be
	 * called prior to calling {@link #compareLowerSubstr(String)} to set the range of the substring
	 * comparison.
	 * 
	...
	/**
	 * Compares the supplied uppercase string with the contents of the character array, starting at
	 * the offset specified. This is a specialized method to help speed up the FastPageParser
	 * slightly.
	 * <p/>
	 * The supplied string is assumed to contain only uppercase ASCII characters. The offset
	 * indicates the offset into the character array that the comparison should start from.
	 * <p/>
	...
	/**
	 * Grows the internal array by either ~100% or minSize (whichever is larger), up to a maximum
