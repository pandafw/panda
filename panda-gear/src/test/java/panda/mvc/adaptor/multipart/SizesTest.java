...
public class SizesTest extends FileUploadTestCase {
	...
	public void testFileUpload() throws IOException {
		...
		List<TestFileItem> fileItems = parseUpload(baos.toByteArray());
	...
	public void testFileSizeLimit() throws IOException {
		...
		List<TestFileItem> fileItems = parseUpload(upload, req);
		assertEquals(1, fileItems.size());
		
		TestFileItem item = fileItems.get(0);
		assertEquals("This is the content of the file\n", new String(item.getBody()));

		upload = new FileUploader();
		upload.setFileSizeMax(40);
		req = new MockHttpServletRequest(request.getBytes("US-ASCII"), CONTENT_TYPE);
		fileItems = parseUpload(upload, req);
		assertEquals(1, fileItems.size());
