...
<#if result?has_content>
	...
	<div class="p-tcenter" focusme="true">
		<@p.submit onclick="return pages_bdelete_submit();" icon="icon-bdelete-execute" label="#(btn-bdelete-execute)"/>

		<@p.a href="javascript:window.history.back()" btn="default" icon="icon-back" label="#(btn-back)"/>

		<script type="text/javascript"><!--
			function pages_bdelete_submit() {
				return plv_submitCheckedKeys('pages_bdelete');
			}
			
			function onPageLoad() {
				plv_checkAll('pages_bdelete');
			}
		--></script>
	</div>
<#else>
