...
<#if result?has_content>
	...
	<div class="p-tcenter" focusme="true">
		<@p.submit onclick="return pages_benable_submit();" icon="icon-benable-execute" label="#(btn-benable-execute)"/>

		<@p.a href="javascript:window.history.back()" btn="default" icon="icon-back" label="#(btn-back)"/>

		<script type="text/javascript"><!--
			function pages_benable_submit() {
				return plv_submitCheckedKeys('pages_benable');
			}
			
			function onPageLoad() {
				plv_checkAll('pages_benable');
			}
		--></script>
	</div>
<#else>
