...
import panda.net.ProtocolCommandEvent;
import panda.net.imap.IMAP;
import panda.net.imap.IMAPClient;
import panda.net.imap.IMAPReply;
import panda.net.imap.IMAP.IMAPChunkListener;

/**
 * This is an example program demonstrating how to use the IMAP[S]Client class. This program
 * connects to a IMAP[S] server and exports selected messages from a folder into an mbox file.
 * <p>
 * Usage: IMAPExportMbox imap[s]://user:password@host[:port]/folder/path <mboxfile> [sequence-set]
 * [item-names]
 * <p>
 ...
 * <p>
 * For example:<br>
 * IMAPExportMbox imaps://username:password@imap.googlemail.com/messages_for_export exported.mbox
 * 1:10,20<br>
 * IMAPExportMbox imaps://username:password@imap.googlemail.com/messages_for_export exported.mbox 3
 * ENVELOPE<br>
 * <p>
 * Note that the sequence-set is passed unmodified to the FETCH command. The item names are wrapped
 * in parentheses if more than one is provided. Otherwise, the parameter is assumed to be wrapped if
 * necessary. Parameters with spaces must be quoted otherwise the OS shell will normally treat them
 * as separate parameters. Also the listener that writes the mailbox only captures the multi-line
 * responses. It does not capture the output from ENVELOPE commands.
 */
public final class IMAPExportMbox {
	...
	// e.g. * 382 EXISTS
	private static final Pattern PATEXISTS = Pattern.compile("\\* (\\d+) EXISTS"); // Response from
																					// SELECT
	...
	public static void main(String[] args) throws IOException {
		...
		final PrintCommandListener listener = new PrintCommandListener(System.out, true) {
			...
			public void protocolReplyReceived(ProtocolCommandEvent event) {
				if (event.getReplyCode() != IMAPReply.PARTIAL) { // This is dealt with by the chunk
																	// listener
	...
	private static class MboxListener implements IMAPChunkListener {
		...
		public boolean chunkReceived(IMAP imap) {
			...
			if (checkSequence) {
				...
				if (m.lookingAt()) { // found a match
					final long msgSeq = Long.parseLong(m.group(PATSEQ_SEQUENCE_GROUP)); // Cannot
																						// fail to
																						// parse
					if (lastSeq != -1) {
