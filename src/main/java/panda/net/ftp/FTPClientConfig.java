 ...
 * <li>lists files with timestamps that use date formats other than the American English "standard"
 ...
 * <code>MMM d yyyy</code> date formatting
 * 
 * <pre>
...
public class FTPClientConfig {
	 ...
	 * to which the client connects. Should be either one of the <code>FTPClientConfig.SYST_*</code>
	 ...
	 * format which will be in as used in <code>en_US</code> locales.
	 * </p>
	 * <p>
	 ...
	 * </p>
	 * <p>
	 * This should be one of the identifiers used by <code>java.util.TimeZone</code> to refer to
	 * time zones, for example, <code>America/Chicago</code> or <code>Asia/Rangoon</code>.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * setter for the serverLanguageCode property. This property allows user to specify a <a
	 * href="http://www.ics.uci.edu/pub/ietf/http/related/iso639.txt"> two-letter ISO-639 language
	 * code</a> that will be used to configure the set of month names used by the file timestamp
	 ...
	 * </p>
	 * <p>
	 * If the code supplied is not supported here, <code>en_US</code> month names will be used. We
	 * are supporting here those language codes which, when a <code> java.util.Locale</code> is
	 * constucted using it, and a <code>java.text.SimpleDateFormat</code> is constructed using that
	 * Locale, the array returned by the SimpleDateFormat's <code>getShortMonths()</code> method
	 ...
	 * </p>
	 * <p>
	 * <strong> Please note that this attribute will NOT be used to determine a locale-based date
	 * format for the language. </strong> Experience has shown that many if not most FTP servers
	 * outside the United States employ the standard <code>en_US</code> date format orderings of
	 * <code>MMM d yyyy</code> and <code>MMM d HH:mm</code> and attempting to deduce this
	 ...
	 *         supplied code, or with month names for <code>Locale.US</code> if there is no
