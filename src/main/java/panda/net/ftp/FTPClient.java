 ...
 * failure.
 * <p>
 * The default settings for FTPClient are for it to use <code> FTP.ASCII_FILE_TYPE </code>,
 * <code> FTP.NON_PRINT_TEXT_FORMAT </code>, <code> FTP.STREAM_TRANSFER_MODE </code>, and
 * <code> FTP.FILE_STRUCTURE </code>. The only file types directly supported are
 * <code> FTP.ASCII_FILE_TYPE </code> and <code> FTP.BINARY_FILE_TYPE </code>. Because there are at
 ...
 * <p>
 * <code> FTP.NON_PRINT_TEXT_FORMAT </code>, <code> FTP.STREAM_TRANSFER_MODE </code>, and
 * <code> FTP.FILE_STRUCTURE </code> are the only supported formats, transfer modes, and file
 * structures.
 * <p>
 ...
 * {@link panda.net.ftp.FTPConnectionClosedException} . <code>FTPConnectionClosedException</code> is
 ...
 * <li>are in different timezones and you need accurate timestamps for dependency checking as in Ant
 * </li>
 * </ul>
...
public class FTPClient extends FTP implements Configurable {
	 ...
	 * {@link Class#getResourceAsStream(String)}.<br>
	 ...
	 * set to <code> ACTIVE_LOCAL_DATA_CONNECTION_MODE </code>, the file type set to
	 ...
	 * i.e. {@code 257<space>"<directory-name>"[<space>commentary]} where any double-quotes in
	 * {@code <directory-name>} are doubled. Unlike MKD, the commentary is optional.
	 * <p>
	 ...
	 * will be set immediately after opening the data connection, provided that the value is &ge; 0.
	 * <p>
	 * <b>Note:</b> the timeout will also be applied when calling accept() whilst establishing an
	...
	/**
	 * Set the current data connection mode to <code> PASSIVE_LOCAL_DATA_CONNECTION_MODE </code>.
	 ...
	 * <code> PASSIVE_LOCAL_DATA_CONNECTION_MODE </code> until the mode is changed by calling some
	...
	/**
	 * Set the current data connection mode to <code> ACTIVE_REMOTE_DATA_CONNECTION </code>. Use
	...
	/**
	 * Set the current data connection mode to <code> PASSIVE_REMOTE_DATA_CONNECTION_MODE </code>.
	 ...
	 * <code> FTP.ASCII_FILE_TYPE </code>, <code> FTP.BINARY_FILE_TYPE</code>, etc. The file type
	 ...
	 * BINARY. <b>To ensure correct operation with all servers, always specify the appropriate file
	 * type after connecting to the server.</b> <br>
	 * <p>
	 ...
	 * BINARY. <b>To ensure correct operation with all servers, always specify the appropriate file
	 * type after connecting to the server.</b> <br>
	 * The format should be one of the FTP class <code> TEXT_FORMAT </code> constants, or if the
	 * type is <code> FTP.LOCAL_FILE_TYPE </code>, the format should be the byte size for that type.
	 * The default format is <code> FTP.NON_PRINT_TEXT_FORMAT </code> if this method is never
	 * called.
	 * <p>
	 * <b>N.B.</b> currently calling any connect method will reset the type to FTP.ASCII_FILE_TYPE
	...
	/**
	 * Sets the file structure. The default structure is <code> FTP.FILE_STRUCTURE </code> if this
	 * method is never called or if a connect method is called.
	 * 
	 * @param structure The structure of the file (one of the FTP class <code>_STRUCTURE</code>
	 *            constants).
	 * @return True if successfully completed, false if not.
	 * @exception FTPConnectionClosedException If the FTP server prematurely closes the connection
	 *                as a result of the client being idle or some other reason causing the server
	...
	/**
	 * Sets the transfer mode. The default transfer mode <code> FTP.STREAM_TRANSFER_MODE </code> if
	 * this method is never called or if a connect method is called.
	 * 
	 * @param mode The new transfer mode to use (one of the FTP class <code>_TRANSFER_MODE</code>
	 *            constants).
	 * @return True if successfully completed, false if not.
	 * @exception FTPConnectionClosedException If the FTP server prematurely closes the connection
	 *                as a result of the client being idle or some other reason causing the server
	 ...
	 * must have had a <code> remoteRetrieve </code> issued to it by another FTPClient.
	 * 
	 ...
	 * filename. The other server must have had a <code> remoteRetrieve </code> issued to it by
	 * another FTPClient.
	 * 
	 ...
	 * must have had a <code> remoteRetrieve </code> issued to it by another FTPClient. Many FTP
	 * servers require that a base filename be given from which the unique filename can be derived.
	 * For those servers use the other version of <code> remoteStoreUnique</code>
	 * 
	 * @return True if successfully completed, false if not.
	 * @exception FTPConnectionClosedException If the FTP server prematurely closes the connection
	 *                as a result of the client being idle or some other reason causing the server
	 ...
	 * completePendingCommand } and check its return value to verify success.</b> If this is not
	...
	/**
	 * Restart a <code>STREAM_TRANSFER_MODE</code> file transfer starting from the given offset.
	 ...
	 * </p>
	 * <p>
	 * <b>Note: This method should only be invoked immediately prior to the transfer to which it
	 * applies.</b>
	 * 
	 ...
	 *         <p>
	 *         <b> NOTE:</b> This array may contain null members if any of the individual file
	 ...
	 *         in the given path in the format determined by the <code> parser </code> parameter.
	 ...
	 *         given path in the format determined by the <code> parser </code> parameter. Null will
	 ...
	 *            <code> FTPFileEntryParser </code> that should be used to parse each server file
	 ...
	 * of a file. The timestamp should be in the form <code>YYYYMMDDhhmmss</code>. It should also be
	 ...
	 * <code>LIST -a</code> will be issued to the ftp server. It depends on your ftp server if you
