...
/***
 * NNTPClient encapsulates all the functionality necessary to post and retrieve articles from an
 * NNTP server. As with all classes derived from {@link panda.net.SocketClient}, you must first
 * connect to the server with {@link panda.net.SocketClient#connect connect } before doing anything,
 * and finally {@link panda.net.nntp.NNTP#disconnect disconnect() } after you're completely finished
 * interacting with the server. Remember that the {@link panda.net.nntp.NNTP#isAllowedToPost
 * isAllowedToPost()} method is defined in {@link panda.net.nntp.NNTP}.
 * <p>
 ...
 * <code>NNTPConectionClosedException</code> is a subclass of <code> IOException </code> and
 ...
 * connection with {@link panda.net.nntp.NNTP#disconnect disconnect() } to properly clean up the
 * system resources used by NNTP. Before disconnecting, you may check the last reply code and text
 * with {@link panda.net.nntp.NNTP#getReplyCode getReplyCode } and
 * {@link panda.net.nntp.NNTP#getReplyString getReplyString }.
 * <p>
...
public class NNTPClient extends NNTP {
	...
	/*
	 * 211 n f l s group selected (n = estimated number of articles in group, f = first article
	 * number in the group, l = last article number in the group, s = name of the group.)
	 */
	 ...
	 * @return the parsed {@link Article}, if unparseable then isDummy() will be true, and the
	 *         subject will contain the raw info.
	 * @since 3.0
	 */
	 ...
	 * <code> articleId </code> field of the ArticleInfo cannot always be trusted because some NNTP
	...
	/**
	 * Same as <code> retrieveArticle(articleId, (ArticleInfo) null) </code> Note: the return can be
	...
	/**
	 * Same as <code> retrieveArticle((String) null) </code> Note: the return can be cast to a
	 * {@link BufferedReader}
	 * 
	 * @return A DotTerminatedMessageReader instance from which the article can be read. null if the
	 *         article does not exist.
	 ...
	 * through an ArticleInfo. The <code> articleId </code> field of the ArticleInfo cannot always
	...
	/**
	 * Same as <code> retrieveArticleHeader(articleId, (ArticleInfo) null) </code> Note: the return
	...
	/**
	 * Same as <code> retrieveArticleHeader((String) null) </code> Note: the return can be cast to a
	 * {@link BufferedReader}
	 * 
	 * @return the reader
	 * @throws IOException if an error occurs
	 ...
	 * returned through an ArticleInfo. The <code> articleId </code> field of the ArticleInfo cannot
	...
	/**
	 * Same as <code> retrieveArticleBody(articleId, (ArticleInfo) null) </code> Note: the return
	 * can be cast to a {@link BufferedReader}
	 * 
	...
	/**
	 * Same as <code> retrieveArticleBody(null) </code> Note: the return can be cast to a
	 * {@link BufferedReader}
	 * 
	 * @return A DotTerminatedMessageReader instance from which the article body can be read. null
	 *         if the article does not exist.
	...
	/****
	 * Same as <code> selectArticle((String) null, articleId) </code>. Useful for retrieving the
	 ...
	 * article identifier, issue a <code> selectArticle(pointer.articleNumber, pointer) </code>
