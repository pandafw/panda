...
/***
 * The NewGroupsOrNewsQuery class. This is used to issue NNTP NEWGROUPS and NEWNEWS queries,
 * implemented by {@link panda.net.nntp.NNTPClient#listNewNewsgroups listNewNewsGroups } and
 * {@link panda.net.nntp.NNTPClient#listNewNews listNewNews } respectively. It prevents you from
 * having to format date, time, distribution, and newgroup arguments.
 * <p>
 ...
 * </pre>
 * 
 * This will retrieve the list of newsgroups starting with the comp. distribution prefix created
 * since midnight 11/15/97.
 * 
 * @see NNTPClient
 ***/
...
public final class NewGroupsOrNewsQuery {
	...
	private StringBuilder __distributions;
	...
	/***
	 * Add a newsgroup to the list of newsgroups being queried. Newsgroups added this way are only
	 * meaningful to the NEWNEWS command. Newsgroup names may include the <code> * </code> wildcard,
	 * as in <code>comp.lang.* </code> or <code> comp.lang.java.* </code>. Adding at least one
	 * newsgroup is mandatory for the NEWNEWS command.
	 * <p>
	 * 
	...
	/***
	 * Add a newsgroup to the list of newsgroups being queried, but indicate that group should not
	 * be checked for new news. Newsgroups added this way are only meaningful to the NEWNEWS
	 * command. Newsgroup names may include the <code> * </code> wildcard, as in
	 * <code>comp.lang.* </code> or <code> comp.lang.java.* </code>.
	 * <p>
	 * The following would create a query that searched for new news in all comp.lang.java
	 * newsgroups except for comp.lang.java.advocacy.
	 * 
	 * <pre>
	 ...
	 * <p>
	 * 
	 * @param newsgroup The newsgroup to add to the list of groups to be checked for new news, but
	 *            which should be omitted from the search for new news..
	 ***/
	...
	/***
	 * Add a distribution group to the query. The distribution part of a newsgroup is the segment of
	 * the name preceding the first dot (e.g., comp, alt, rec). Only those newsgroups matching one
	 * of the distributions or, in the case of NEWNEWS, an article in a newsgroup matching one of
	 * the distributions, will be reported as a query result. Adding distributions is purely
	 * optional.
	 * <p>
	 * 
	...
	public void addDistribution(String distribution) {
		if (__distributions != null) {
			...
			__distributions = new StringBuilder();
		}
	...
	/***
	 * Return the comma separated list of distributions. This may be null if there are no
	 * distributions.
	 * <p>
	 * 
