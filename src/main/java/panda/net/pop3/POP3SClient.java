 ...
 * execTLS() and the server accepts the command. Implicit usage: POP3SClient c = new
 * POP3SClient(true); c.connect("127.0.0.1", 995); Explicit usage: POP3SClient c = new
 * POP3SClient(); c.connect("127.0.0.1", 110); if (c.execTLS()) { /rest of the commands here/ }
...
public class POP3SClient extends POP3Client {
	...
	/**
	 * Constructor for POP3SClient, using {@link #DEFAULT_PROTOCOL} i.e. TLS Sets security mode to
	 * explicit.
	 */
	public POP3SClient() {
	...
	/**
	 * Constructor for POP3SClient. Sets the default port to {@link #DEFAULT_POP3S_PORT} - 995 - if
	 * using implicit mode
	 * 
	 * @param proto the protocol.
	 * @param implicit The security mode, {@code true} for implicit, {@code false} for explicit
