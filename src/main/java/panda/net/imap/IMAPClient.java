...
public class IMAPClient extends IMAP {
	...
	/**
	 * Send a NOOP command to the server. This is useful for keeping a connection alive since most
	 * IMAP servers will timeout after 10 minutes of inactivity.
	 * 
	 * @return {@code true} if the command was successful,{@code false} if not.
	 * @exception IOException If a network I/O error occurs.
	 */
	...
	/**
	 * Send a LOGOUT command to the server. To fully disconnect from the server you must call
	 * disconnect(). A logout attempt is valid in any state. If the client is in the not
	 * authenticated or authenticated state, it enters the logout on a successful logout.
	 * 
	 * @return {@code true} if the command was successful,{@code false} if not.
	 * @exception IOException If a network I/O error occurs.
	 */
	...
	/**
	 * Login to the IMAP server with the given username and password. You must first connect to the
	 * server with {@link panda.net.SocketClient#connect connect } before attempting to login. A
	 * login attempt is only valid if the client is in the NOT_AUTH_STATE. After logging in, the
	 * client enters the AUTH_STATE.
	 * 
	 ...
	 * @return {@code true} if the command was successful,{@code false} if not.
	 * @exception IOException If a network I/O error occurs.
	 * @deprecated (3.4) Does not work; the message body is not optional. Use
	 *             {@link #append(String, String, String, String)} instead.
	 */
	@Deprecated
	 ...
	 * @param itemNames The item names for the FETCH command. (e.g. BODY.PEEK[HEADER.FIELDS
	 *            (SUBJECT)]) If multiple item names are requested, these must be enclosed in
	...
	public enum SEARCH_CRITERIA {
		...
		BCC,
		/**
		 * Messages whose internal date (disregarding time and timezone) is earlier than the
		 * specified date.
		 */
		BEFORE,
		/**
		...
		FROM,
		/**
		 * Messages that have a header with the specified field-name (as defined in [RFC-2822]) and
		 * that contains the specified string in the text of the header (what comes after the
		 * colon). If the string to search is zero-length, this matches all messages that have a
		 * header line with the specified field-name regardless of the contents.
		 */
		HEADER,
		...
		LARGER,
		/**
		 * Messages that have the \Recent flag set but not the \Seen flag. This is functionally
		...
		NOT,
		/**
		 * Messages that do not have the \Recent flag set. This is functionally equivalent to
		 * "NOT RECENT" (as opposed to "NOT NEW").
		 */
		OLD,
		/**
		 * Messages whose internal date (disregarding time and timezone) is within the specified
		 * date.
		 */
		ON,
		...
		SEEN,
		/**
		 * Messages whose [RFC-2822] Date: header (disregarding time and timezone) is earlier than
		 * the specified date.
		 */
		SENTBEFORE,
		/**
		 * Messages whose [RFC-2822] Date: header (disregarding time and timezone) is within the
		 * specified date.
		 */
		SENTON,
		/**
		 * Messages whose [RFC-2822] Date: header (disregarding time and timezone) is within or
