...
public final class IMAPReply {
	...
	/**
	 * The reply code indicating a partial response. This is used when a chunk listener is
	 * registered and the listener requests that the reply lines are cleared on return.
	 * 
	 * @since 3.4
	 */
	...
	private static final String TAGGED_RESPONSE = "^\\w+ (\\S+).*"; // TODO perhaps be less strict
																	// on tag match?
	...
	private static int getReplyCode(String line, Pattern pattern) throws IOException {
		...
		if (m.matches()) { // TODO would lookingAt() be more efficient? If so, then drop trailing .*
							// from patterns
