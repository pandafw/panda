...
/***
 * SMTP provides the basic the functionality necessary to implement your own SMTP client. To derive
 * the full benefits of the SMTP class requires some knowledge of the FTP protocol defined in RFC
 * 821. However, there is no reason why you should have to use the SMTP class. The
 * {@link panda.net.smtp.SMTPClient} class, derived from SMTP, implements all the functionality
 * required of an SMTP client. The SMTP class is made public to provide access to various SMTP
 * constants and to make it easier for adventurous programmers (or those with special needs) to
 * interact with the SMTP protocol and implement their own clients. A set of methods with names
 * corresponding to the SMTP command names are provided to facilitate this interaction.
 * <p>
 * You should keep in mind that the SMTP server may choose to prematurely close a connection for
 * various reasons. The SMTP class will detect a premature SMTP server connection closing when it
 * receives a {@link panda.net.smtp.SMTPReply#SERVICE_NOT_AVAILABLE SMTPReply.SERVICE_NOT_AVAILABLE
 * } response to a command. When that occurs, the SMTP class method encountering that reply will
 * throw an {@link panda.net.smtp.SMTPConnectionClosedException} .
 * <code>SMTPConectionClosedException</code> is a subclass of <code> IOException </code> and
 * therefore need not be caught separately, but if you are going to catch it separately, its catch
 * block must appear before the more general <code> IOException </code> catch block. When you
 * encounter an {@link panda.net.smtp.SMTPConnectionClosedException} , you must disconnect the
 * connection with {@link panda.net.SocketClient#disconnect disconnect() } to properly clean up the
 * system resources used by SMTP. Before disconnecting, you may check the last reply code and text
 * with {@link #getReplyCode getReplyCode }, {@link #getReplyString getReplyString }, and
...
public class SMTP extends SocketClient {
	...
	/***
	 * The default SMTP constructor. Sets the default port to <code>DEFAULT_PORT</code> and
	 * initializes internal data structures for saving SMTP reply information.
	 ***/
	public SMTP() {
	...
	/***
	 * Closes the connection to the SMTP server and sets to null some internal data so that the
	...
	/***
	 * Sends an SMTP command to the server, waits for a reply and returns the numerical response
	 ...
	 * @param args The arguments to the SMTP command. If this parameter is set to null, then the
	 *            command is sent with no argument.
	 * @return The integer value of the SMTP reply code returned by the server in response to the
	 *         command.
	...
	/***
	 * Sends an SMTP command with no arguments to the server, waits for a reply and returns the
	...
	/***
	 * Returns the integer value of the reply code of the last SMTP reply. You will usually only use
	 * this method after you connect to the SMTP server to check that the connection was successful
	...
	/***
	 * Fetches a reply from the SMTP server and returns the integer reply code. After calling this
	 ...
	 * are implementing your own SMTP client or if you need to fetch a secondary response from the
	 * SMTP server.
	 * <p>
	 * 
	...
	/***
	 * Returns the lines of text from the last SMTP server response as an array of strings, one
	 * entry per line. The end of line markers of each are stripped from each line.
	 * <p>
	 * 
	...
	/***
	 * Returns the entire text of the last SMTP server response exactly as it was received,
	...
	/***
	 * A convenience method to send the SMTP HELO command to the server, receive the reply, and
	 * return the reply code.
	 * <p>
	 * 
	...
	/***
	 * A convenience method to send the SMTP MAIL command to the server, receive the reply, and
	 * return the reply code.
	 * <p>
	 * 
	...
	/***
	 * A convenience method to send the SMTP RCPT command to the server, receive the reply, and
	 * return the reply code.
	 * <p>
	 * 
	...
	/***
	 * A convenience method to send the SMTP DATA command to the server, receive the reply, and
	 * return the reply code.
	 * <p>
	 * 
	 * @return The reply code received from the server.
	...
	/***
	 * A convenience method to send the SMTP SEND command to the server, receive the reply, and
	 * return the reply code.
	 * <p>
	 * 
	 * @param reversePath The reverese path.
	...
	/***
	 * A convenience method to send the SMTP SOML command to the server, receive the reply, and
	 * return the reply code.
	 * <p>
	 * 
	 * @param reversePath The reverese path.
	...
	/***
	 * A convenience method to send the SMTP SAML command to the server, receive the reply, and
	 * return the reply code.
	 * <p>
	 * 
	 * @param reversePath The reverese path.
	...
	/***
	 * A convenience method to send the SMTP RSET command to the server, receive the reply, and
	 * return the reply code.
	 * <p>
	 * 
	 * @return The reply code received from the server.
	...
	/***
	 * A convenience method to send the SMTP VRFY command to the server, receive the reply, and
	 * return the reply code.
	 * <p>
	 * 
	...
	/***
	 * A convenience method to send the SMTP HELP command to the server, receive the reply, and
	 * return the reply code.
	 * <p>
	 * 
	 * @return The reply code received from the server.
	...
	/***
	 * A convenience method to send the SMTP NOOP command to the server, receive the reply, and
	 * return the reply code.
	 * <p>
	 * 
	 * @return The reply code received from the server.
	...
	/***
	 * A convenience method to send the SMTP TURN command to the server, receive the reply, and
	 * return the reply code.
	 * <p>
	 * 
	 * @return The reply code received from the server.
	...
	/***
	 * A convenience method to send the SMTP QUIT command to the server, receive the reply, and
	 * return the reply code.
	 * <p>
	 * 
	 * @return The reply code received from the server.
	...
	/**
	 * Removes a ProtocolCommandListener. Delegates this incorrectly named method -
	 * removeProtocolCommandistener (note the missing "L")- to the correct method
	 * {@link SocketClient#removeProtocolCommandListener}
	 * 
	 * @param listener The ProtocolCommandListener to remove
	 */
	public void removeProtocolCommandistener(panda.net.ProtocolCommandListener listener) {
