...
/**
 * SMTP over SSL processing. Copied from FTPSClient.java and modified to suit SMTP. If implicit mode
 * is selected (NOT the default), SSL/TLS negotiation starts right after the connection has been
 * established. In explicit mode (the default), SSL/TLS negotiation starts when the user calls
 * execTLS() and the server accepts the command. Implicit usage: SMTPSClient c = new
 * SMTPSClient(true); c.connect("127.0.0.1", 465); Explicit usage: SMTPSClient c = new
 * SMTPSClient(); c.connect("127.0.0.1", 25); if (c.execTLS()) { /rest of the commands here/ }
...
public class SMTPSClient extends SMTPClient {
	...
	/**
	 * The cipher suites. SSLSockets have a default set of these anyway, so no initialization
	 * required.
	 */
	...
	/**
	 * Constructor for SMTPSClient, using {@link #DEFAULT_PROTOCOL} i.e. TLS Sets security mode to
	 ...
	 * connection. When the underlying {@link java.net.Socket Socket} is not an {@link SSLSocket}
	 ...
	 * 
	 * @throws IOException If an I/O error occurs while sending the command or performing the
	 *             negotiation.
	 ...
	 * enabled. The default behaviour is for this to be disabled.
	 * 
	 ...
	 * 1.7+. The default behaviour is for this to be disabled.
	 * 
