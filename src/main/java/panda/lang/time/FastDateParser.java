 ...
 * </p>
 * <p>
 * This class can be used as a direct replacement for <code>SimpleDateFormat</code> in most parsing
 * situations. This class is especially useful in multi-threaded server environments.
 * <code>SimpleDateFormat</code> is not thread-safe in any JDK version, nor will it be as Sun have
 * closed the <a href="http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4228335">bug</a>/RFE.
 * </p>
 * <p>
...
public class FastDateParser implements DateParser, Serializable {
	...
	private static StringBuilder escapeRegex(final StringBuilder regex, final String value, final boolean unquote) {
		...
		for (int i = 0; i < value.length(); ++i) {
			...
			case '\\':
				...
				/*
				 * If we have found \E, we replace it with \E\\E\Q, i.e. we stop the quoting, quote
				 * the \ in \E, then restart the quoting. Otherwise we just output the two
				 * characters. In each case the initial \ needs to be output and the final char is
				 * done at the end
				 */
	...
	private static abstract class Strategy {
		/**
		 * Is this field a number? The default implementation returns true.
		 * 
	...
	private static final Pattern formatPattern = Pattern
		.compile("D+|E+|F+|G+|H+|K+|M+|S+|W+|Z+|a+|d+|h+|k+|m+|s+|w+|y+|z+|''|'[^']++(''[^']*+)*+'|[^'A-Za-z]++");
