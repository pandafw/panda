...
public class Methods {
	private static Log log = Logs.getLog(Methods.class);

	/**
	 * Only log warning about accessibility work around once.
	 * <p>
	 * Note that this is broken when this class is deployed via a shared classloader in a container,
	 * as the warning message will be emitted only once, not once per webapp. However making the
	 * warning appear once per webapp means having a map keyed by context classloader which
	 * introduces nasty memory-leak problems. As this warning is really optional we can ignore this
	 * problem; only one of the webapps will get the warning in its logs but that should be good
	 * enough.
	 */
	 ...
	 * <p>
	 * Note that when this class is deployed via a shared classloader in a container, this will
	 * affect all webapps. However making this configurable per webapp would mean having a map keyed
	 * by context classloader which may introduce memory-leak problems.
	 */
	 ...
	 * <p>
	 * The keys into this map only ever exist as temporary variables within methods of this class,
	 * and are never exposed to users of this class. This means that the WeakHashMap is used only as
	 * a mechanism for limiting the size of the cache, ie a way to tell the garbage collector that
	 * the contents of the cache can be completely garbage-collected whenever it needs the memory.
	 * Whether this is a good approach to this problem is doubtful; something like the
	 * commons-collections LRUMap may be more appropriate (though of course selecting an appropriate
	 * size is an issue).
	 * <p>
	 * This static variable is safe even when this code is deployed via a shared classloader because
	 * it is keyed via a MethodDescriptor object which has a Class as one of its members and that
	 * member is used in the MethodDescriptor.equals method. So two components that load the same
	 * class via different classloaders will generate non-equal MethodDescriptor objects and hence
	 * end up with different entries in the map.
	 */
	 ...
	 * <code>true</code>.
	 * 
	 * @param cacheMethods <code>true</code> if methods should be cached for greater performance,
	 ...
	 * and a single parameter. If no such method can be found, return <code>null</code>. Basically,
	 * a convenience wrapper that constructs a <code>Class</code> array for you.
	 * </p>
	 * 
	...
	private static void setMethodAccessible(Method method) {
		try {
			...
			if (!loggedAccessibleWarning) {
				...
				try {
					String specVersion = Systems.JAVA_SPECIFICATION_VERSION;
	...
	/**
	 * <p>
	 * Determine whether a type can be used as a parameter in a method invocation. This method
	 * handles primitive conversions correctly.
	 * </p>
	 * <p>
	 * In order words, it will match a <code>Boolean</code> to a <code>boolean</code>, a
	 * <code>Long</code> to a <code>long</code>, a <code>Float</code> to a <code>float</code>, a
	 * <code>Integer</code> to a <code>int</code>, and a <code>Double</code> to a
	 * <code>double</code>. Now logic widening matches are allowed. For example, a <code>Long</code>
	 * will not match a <code>int</code>.
	 * 
	...
	public static final boolean isAssignmentCompatible(Class parameterType, Class parameterization) {
		...
		if (parameterType.isPrimitive()) {
			...
			Class parameterWrapperClazz = Classes.primitive2Wrapper(parameterType);
	...
	private static class MethodDescriptor {
		...
		/**
		 * Returns the string length of method name. I.e. if the hashcodes are different, the
		 * objects are different. If the hashcodes are the same, need to use the equals method to
		 * determine equality.
		 * 
