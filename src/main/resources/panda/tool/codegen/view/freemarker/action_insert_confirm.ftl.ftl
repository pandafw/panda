...
<#if "true" == props["ui.input.step"]!"">
	${s}#if action.getTextAsBoolean('ui-input-step', false)>
		<ul class="n-step">
			...
			<li class="n-step-next n-step-self">${s}@n.text name="step-${ui.name}_confirm"/></li>
			<li class="n-step-arrow">${s}@n.text name="step-arrow-e">&gt;${s}/@n.text></li>
	...
	${s}/#if>
		...
		${s}@n.form cssClass="n-cform" id="<#if ui.formId?has_content>${ui.formId}<#else>${action.name}</#if>" method="post"<#if ui.theme?has_content> theme="${ui.theme}"</#if>>
			<#include "view-fields.ftl"/>
			${s}@n.submit icon="icon-${ui.name}-execute" action="${action.name}_${ui.name}_execute" group="start">${s}@n.text name="button-${ui.name}-execute"/>${s}/@n.submit>
			${s}@n.submit icon="icon-back" action="${action.name}_${ui.name}_input" group="end">${s}@n.text name="button-back"/>${s}/@n.submit>
		${s}/@n.form>
	</div>
</div>
